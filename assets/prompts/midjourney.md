...
1. **The style anchor already exists** — [`anchor.png`](anchor.png), generated
   ...
   at `--sw 30`, so there is nothing to substitute.
...
```
On a flat solid uniform bright magenta chroma-key background, evenly lit with no shadow anywhere: a single isometric game floor tile of worn grey flagstone marked with faint chalk ward lines, drawn as the flat top face only with no thickness and no side walls, shaped as a 2:1 dimetric diamond exactly twice as wide as it is tall, seen from a fixed 45 degree overhead game camera, tileable, the diamond touching all four edges of the frame --ar 2:1 --style raw --s 50 --c 0 --sw 30 --no shadow, gradient, vignette, objects, text, watermark, frame, border, perspective distortion --v 7 --sref https://cdn.midjourney.com/61968eae-6b14-46db-a1e5-525574866bf0/0_0.png
On a flat solid uniform bright magenta chroma-key background, evenly lit with no shadow anywhere: a single isometric game floor tile of dark polished oak lecture-hall floorboards, drawn as the flat top face only with no thickness and no side walls, shaped as a 2:1 dimetric diamond exactly twice as wide as it is tall, seen from a fixed 45 degree overhead game camera, tileable, the diamond touching all four edges of the frame --ar 2:1 --style raw --s 50 --c 0 --sw 30 --no shadow, gradient, vignette, objects, text, watermark, frame, border, perspective distortion --v 7 --sref https://cdn.midjourney.com/61968eae-6b14-46db-a1e5-525574866bf0/0_0.png
On a flat solid uniform bright magenta chroma-key background, evenly lit with no shadow anywhere: a single isometric game floor tile of stained slate laboratory flooring with faint acid burns and a narrow drainage channel, drawn as the flat top face only with no thickness and no side walls, shaped as a 2:1 dimetric diamond exactly twice as wide as it is tall, seen from a fixed 45 degree overhead game camera, tileable, the diamond touching all four edges of the frame --ar 2:1 --style raw --s 50 --c 0 --sw 30 --no shadow, gradient, vignette, objects, text, watermark, frame, border, perspective distortion --v 7 --sref https://cdn.midjourney.com/61968eae-6b14-46db-a1e5-525574866bf0/0_0.png
On a flat solid uniform bright magenta chroma-key background, evenly lit with no shadow anywhere: a single isometric game floor tile of deep red patterned scriptorium carpet with a gold thread border, drawn as the flat top face only with no thickness and no side walls, shaped as a 2:1 dimetric diamond exactly twice as wide as it is tall, seen from a fixed 45 degree overhead game camera, tileable, the diamond touching all four edges of the frame --ar 2:1 --style raw --s 50 --c 0 --sw 30 --no shadow, gradient, vignette, objects, text, watermark, frame, border, perspective distortion --v 7 --sref https://cdn.midjourney.com/61968eae-6b14-46db-a1e5-525574866bf0/0_0.png
On a flat solid uniform bright magenta chroma-key background, evenly lit with no shadow anywhere: a single isometric game floor tile of scrubbed pale refectory stone with faint spill stains, drawn as the flat top face only with no thickness and no side walls, shaped as a 2:1 dimetric diamond exactly twice as wide as it is tall, seen from a fixed 45 degree overhead game camera, tileable, the diamond touching all four edges of the frame --ar 2:1 --style raw --s 50 --c 0 --sw 30 --no shadow, gradient, vignette, objects, text, watermark, frame, border, perspective distortion --v 7 --sref https://cdn.midjourney.com/61968eae-6b14-46db-a1e5-525574866bf0/0_0.png
On a flat solid uniform bright magenta chroma-key background, evenly lit with no shadow anywhere: a single isometric game floor tile of packed sand and dirt from a training yard with a painted practice circle, drawn as the flat top face only with no thickness and no side walls, shaped as a 2:1 dimetric diamond exactly twice as wide as it is tall, seen from a fixed 45 degree overhead game camera, tileable, the diamond touching all four edges of the frame --ar 2:1 --style raw --s 50 --c 0 --sw 30 --no shadow, gradient, vignette, objects, text, watermark, frame, border, perspective distortion --v 7 --sref https://cdn.midjourney.com/61968eae-6b14-46db-a1e5-525574866bf0/0_0.png
On a flat solid uniform bright magenta chroma-key background, evenly lit with no shadow anywhere: a single isometric game floor tile of cold grey granite inlaid with a brass strip, drawn as the flat top face only with no thickness and no side walls, shaped as a 2:1 dimetric diamond exactly twice as wide as it is tall, seen from a fixed 45 degree overhead game camera, tileable, the diamond touching all four edges of the frame --ar 2:1 --style raw --s 50 --c 0 --sw 30 --no shadow, gradient, vignette, objects, text, watermark, frame, border, perspective distortion --v 7 --sref https://cdn.midjourney.com/61968eae-6b14-46db-a1e5-525574866bf0/0_0.png
On a flat solid uniform bright magenta chroma-key background, evenly lit with no shadow anywhere: a single isometric game floor tile of dark green study carpet worn pale along one path, drawn as the flat top face only with no thickness and no side walls, shaped as a 2:1 dimetric diamond exactly twice as wide as it is tall, seen from a fixed 45 degree overhead game camera, tileable, the diamond touching all four edges of the frame --ar 2:1 --style raw --s 50 --c 0 --sw 30 --no shadow, gradient, vignette, objects, text, watermark, frame, border, perspective distortion --v 7 --sref https://cdn.midjourney.com/61968eae-6b14-46db-a1e5-525574866bf0/0_0.png
On a flat solid uniform bright magenta chroma-key background, evenly lit with no shadow anywhere: a single isometric game floor tile containing the opening of a descending spiral stone stair, blue arcane light glowing up from below, drawn as the flat top face only with no thickness and no side walls, shaped as a 2:1 dimetric diamond exactly twice as wide as it is tall, seen from a fixed 45 degree overhead game camera, the diamond touching all four edges of the frame --ar 2:1 --style raw --s 50 --c 0 --sw 30 --no shadow, gradient, vignette, text, watermark, frame, border, perspective distortion --v 7 --sref https://cdn.midjourney.com/61968eae-6b14-46db-a1e5-525574866bf0/0_0.png
```
...
- **The background comes back muddy or tinted.** That is the style reference
  bleeding into it. Lower `--sw` further (20, then 10). If magenta never
  returns cleanly, switch the background clause to `a pure chroma key green
  screen background` — Midjourney renders that far more reliably, since it is a
  real photographic thing, and the importer samples whatever it gets anyway.

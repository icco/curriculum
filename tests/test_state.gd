...
func test_fail_loop_banks_insight_and_wipes_the_run() -> void:
	...
	state.run["gear"] = {"weapon": {"name": "Oak Quarterstaff"}}
	state.add_consumable({"name": "Vial of Cordial"})
	state.run["depth"] = 4
...
func test_run_state_round_trips_through_the_player_entity() -> void:
	...
	player.current_hp = 7
	player.spend_slot(1)
	player.equipped_gear = {"weapon": {"name": "Iron Stylus", "damage_dice": "1d6"}}
	...
	eq(str(rebuilt.equipped_gear["weapon"]["name"]), "Iron Stylus", "gear restored")

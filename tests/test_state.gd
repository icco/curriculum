...
const TEST_SAVE := "user://loopwood_test_save.json"
...
func test_starting_spells_are_always_known() -> void:
	...
	for id: String in Roster.starting_spells():
		truthy(player.known_spells.has(id), "%s is known from the first loop" % id)
	truthy(player.known_spells.has("chalk_dart"), "the cantrip is available")
	falsy(player.known_spells.has("fire_drill"), "later spells must be unlocked")

func test_skill_tree_gating_and_purchase() -> void:
	var state := GameState.new()
	truthy(state.node_available("study_group"), "a root node is offered")
	falsy(state.node_available("second_period"), "a gated node is hidden")
	falsy(state.can_afford("study_group"), "cannot buy without insight")
	falsy(state.purchase_node("study_group"), "purchase refused when broke")

	state.global["skill_points"] = 10
	truthy(state.purchase_node("study_group"), "purchase succeeds")
	eq(int(state.global["skill_points"]), 8, "insight is deducted")
	eq(int(state.global["bonus_hp"]), 4, "the effect is applied")
	falsy(state.node_available("study_group"), "nodes cannot be bought twice")
	falsy(state.purchase_node("study_group"), "and the purchase is refused")

	truthy(state.purchase_node("extra_period"), "another root node")
	truthy(state.node_available("second_period"), "prerequisites now met")

func test_spell_unlock_nodes_teach_spells_permanently() -> void:
	var state := GameState.new()
	state.global["skill_points"] = 20
	truthy(state.purchase_node("pop_quiz_node"), "bought the Pop Quiz node")
	truthy((state.global["unlocked_spells"] as Array).has("pop_quiz"), "spell recorded in GlobalState")
	var player := Roster.make_player(state.global)
	truthy(player.known_spells.has("pop_quiz"), "the next protagonist knows it")

	state.fail_loop()
	var next := Roster.make_player(state.global)
	truthy(next.known_spells.has("pop_quiz"), "and still knows it after the loop resets")

func test_stat_node_applies_to_the_named_ability() -> void:
	var state := GameState.new()
	state.global["skill_points"] = 30
	state.purchase_node("pop_quiz_node")
	truthy(state.purchase_node("honour_roll"), "Honour Roll requires Pop Quiz")
...
func test_fail_loop_banks_insight_and_wipes_the_run() -> void:
	...
	state.run["gear"] = {"weapon": {"name": "Hockey Stick"}}
	state.add_consumable({"name": "Juice Box"})
	state.run["depth"] = 4
	...
	truthy(state.consumables().is_empty(), "consumables lost")
...
func test_win_sets_the_escape_flag() -> void:
	...
	truthy(bool(state.story_flag("escaped_school")), "story flag set")
...
func test_run_state_round_trips_through_the_player_entity() -> void:
	...
	player.current_hp = 7
	player.spend_slot(1)
	player.equipped_gear = {"weapon": {"name": "Metal Ruler", "damage_dice": "1d6"}}
	...
	eq(str(rebuilt.equipped_gear["weapon"]["name"]), "Metal Ruler", "gear restored")
...
func test_save_and_load_round_trip() -> void:
	...
	state.global["unlocked_spells"] = ["pop_quiz", "hall_pass"]
	state.global["loops"] = 7
	state.set_story_flag("beat_sub_teacher_floor_1")
	state.run["depth"] = 3
	state.run["hp"] = 9
	...
	truthy(bool(loaded.story_flag("beat_sub_teacher_floor_1")), "story flags persisted")
